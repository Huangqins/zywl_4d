    ...
    <div class="queryCriteria">
      ...
      <div>
        <el-select v-model="taskname" filterable placeholder="任务名称">
          <el-option v-for="item in tasknameS" :key="item.value" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
        <el-select v-model="scanTarget" filterable placeholder="扫描目标" class="select">
          <el-option v-for="item in scanTargetS" :key="item.value" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
      ...
      <div>
        <el-button type="primary" @click="addDialog">
          <i class="el-icon-plus"></i>添加任务测试 </el-button>
      </div>
    </div>
    <div>
      <div class="task-tab">
        ...
        <panel>
          <el-table :data="tableData" style="width: 100%;">
            ...
            <el-table-column label="执行状态" align="center" width="80">
              <template slot-scope="scope">
                {{ taskStatus[scope.row.target_struts] }}
              </template>
            </el-table-column>
